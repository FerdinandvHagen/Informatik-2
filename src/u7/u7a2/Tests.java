...
public class Tests {
	...
	{
		IBinarySearchTreeUtils.UnlinkSmallestResult<String> result = utils.unlinkSmallest(tree);
		Assert.assertEquals(get(0).thing, result.smallest.thing);
		checkOrder("[1, 2, 3, 4, 5, 6]", utils.inOrder(result.tree));
	}
	
	@Test public void unlinkLeftList() { testUnlink(leftList()); }
	@Test public void unlinkRightList() { testUnlink(rightList()); }
	@Test public void unlinkBalanced() { testUnlink(balancedTree()); }
	@Test public void unlinkGeneric() { testUnlink(genericTree()); }
